...
fn_handle = @(x) linearStableDS(x, [3;0]);

N = 40;
R = 6;
%x0 = [ones(1,N)*6 ; linspace(-4,4,N)];

phi = linspace(0,2*pi,N);
x0 = [R*cos(phi);R*sin(phi)];
%x0 = x0(:,5);
...
opt_sim.obstacleAvoidanceFunction = @(x,xd,obs,b_contour,varargin) obs_modulation_IFD(x,xd,obs,b_contour, varargin);
...
[X(:,:),Y(:,:)] = meshgrid(linspace(-8,4,N_x), linspace(-4,10,N_y));
...
for ix = 1:N_x
    for iy = 1:N_y
        ...
        if(coll)
            ...
            xd_ifd(:,ix,iy) = obs_modulation_IFD([X(ix,iy);Y(ix,iy)],xd_hat(:,ix,iy), obs1, b_contour, xd_obs,w_obs);
...
relativeChangeSqr_rot = sum(sum(sum((xd_ifd - xd_hat).^2)))/N_tot;
...
quiver(X(:,:), Y(:,:), squeeze(xd_ifd(1,:,:)), squeeze(xd_ifd(2,:,:)), 'r')
...
axis equal;
legend('','','Original', 'Ellipsoid', 'Fluid', 'IFD')
%print('fig/quiverPlot_DS_LS_circle_fluidMechanics','-depsc')
