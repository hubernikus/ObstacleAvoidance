function [x xd t xT x_obs]=Simulation_meshgrid(x0,xT,fn_handle, varargin)
%
...
if dataAnalysis
    penetrationNum = zeros(nbSPoints,1);  % Number of timesteps with penetration
    convergeTime = zeros(nbSPoints,1);  % Discrete integration of the time
    convergeDist = zeros(nbSPoints,1);  % Discrete integration of the distance
    convergeEnergy = zeros(nbSPoints,1);  % Discrete integration of energy
end
...
while true
    ...
    if obs_bool
        ...
        for j=1:nbSPoints % j - iteration over number of starting points
           ...
           [xd(:,iSim,j), b_contour(j)] = obsFunc_handle(x(:,iSim,j),xd(:,iSim,j),obs,b_contour(j),xd_obs, w_obs);
        end
    ...
    for ii = 1:size(x,3)
        if obs_check_collision(obs,x(:,end,ii)) %Observe collision
            % TODO: check tis
            penetrationNum(ii) = penetrationNum(ii) +1 ;
...
%         end 
    ...
    if and(dataAnalysis, iSim > 1)
        for ii = 1:size(x,3)
            if convergeTime(ii) == 0 % not converged yet
                lastDistSqrd =  sum((x(:,end,ii)-x(:,end-1,ii)).^2);
                convergeDist(ii) = convergeDist(ii) + lastDistSqrd;  % Discrete integration of the distance
                convergeEnergy = convergeEnergy +  sqrt(sum((xd(:,end,ii)-xd(:,end-1,ii)).^2)*lastDistSqrd); % Discrete integration of energy

                if all(all(abs(xd_3last(:,:,ii)<options.tol))) % is converged now
                    convergeTime(ii) = iSim*options.dt;
                end
            end
        end
    end
...
if(dataAnalysis)
    badTrajectoriesNum = sum(penetrationNum>0,1);
    fileID = fopen(strcat('simulationResults/table_',simulationName,'.txt'),'w');
    fprintf(fileID,'Numer of Trajecctories with Penetration [] & Penetration Steps [] & Convergence Time [s] & Convergence Distance [m] & Normaliced Energy [J/kg] \\\\ \\hline \n');
    fprintf(fileID,'%3.4f & %3.4f & %3.4f & %3.4f \\\\ \\hline \n', ....
            badTrajectoriesNum, sum(penetrationNum,1)/badTrajectoriesNum, sum(convergeTime,1)/nbSPoints, sum(convergeDist,1)/nbSPoints, sum(convergeEnergy,1)/nbSPoints);
    fclose(fileID);
end
