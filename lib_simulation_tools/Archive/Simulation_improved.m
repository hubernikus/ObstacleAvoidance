function [x xd t xT x_obs]=Simulation_improved(x0,xT,fn_handle,varargin)
%
...
%# create AVI object
% nFrames = 20;
% vidObj = VideoWriter('ani/test', 'MPEG-4')
% vidObj.Quality = 100;
% vidObj.FrameRate = 10;
% open(vidObj);
%         
...
while true
    ...
    if obs_bool
        ...
        merge_obs = obs_merge_simple(obs); % draw counturing object

        if(size(merge_obs,1))
            for n=1:size(merge_obs,3)                        
                obs2{end+1} = merge_obs{n};
            end
        end
        
        for j=1:nbSPoint % j - iteration over number of starting points
            %[xd(:,i,j) b_contour(j)] = obs_modulation_ellipsoid_2(x(:,i,j),xd(:,i,j),obs,b_contour(j),xd_obs);
            
            [xd(:,i,j) b_contour(j)] = obs_modulation_ellipsoid_2(x(:,i,j),xd(:,i,j),obs2,b_contour(j),xd_obs);
        end
...
%         xd(:,i,:)=reshape(fn_handle(squeeze(x(:,i,:))-XT),[d 1 nbSPoint]);
    ...
    %drawToImage()
    if(figSave)
        %saveas(sp.fig,strcat('ani/',figName,'/fig',int2str(i)),'png')
        %print(strcat('ani/',figName,'/fig',int2str(i)),'-dpng')
        %set(sp.fig,'visible','off');
        
        %print(strcat('ani/',figName,'/fig',int2str(i)),'-dpng')
        %print(strcat('ani/',figName,'/fig',int2str(i)),'-depsc')
        
        %saveas(sp.fig, strcat('ani/',figName,'/fig',int2str(i)),'pdf')
        %saveas(sp.fig, strcat('ani/',figName,'/fig',int2str(i)),'eps')
        %saveas(sp.fig, 'test', 'pdf')
    ...
    case 'i' %initializing the figure
        if d==2
            ...
            if b_obs
                ...
                [x_obs, x_obs_sf] = obs_draw_ellipsoid_2(obs,40);
                
                merge_obs = obs_merge_simple(x_obs_sf); % draw counturing object
                
                if(size(merge_obs,1))
                    for n=1:size(merge_obs,3)                        
                        sp.obsMerge_sf(n) = plot(merge_obs(1,:),merge_obs(2,:),'r--','linewidth',2); % !!! ONLY for two objects at the moment
                    end
                end
        ...
        elseif d==3
            ...
            if b_obs
                n_theta = 15;
                n_phi = 10;
                x_obs = obs_draw_ellipsoid_2(obs,[n_theta n_phi]);
                for n=1:size(x_obs,3)
    ...
    case 'o' %updating the obstacle position
        ...
        if b_obs
            ...
            if d==2
                ...
                %upObj_sf = [get(sp.obs_sf(n),'XData')+ dx(1);get(sp.obs_sf(n),'YData')+ dx(2)];
                %set(sp.obs_sf(n),'XData',upObj_sf(1,:,n))
                %set(sp.obs_sf(n),'YData',upObj_sf(2,:,n))
                
                % Delete old obs merge
                if(sp.obsMerge_sf(n))
                    for i =1:size(sp.obsMerge_sf,3)
                        delete1 = get(sp.obsMerge_sf(n), 'delete1');
                        delete(delete1);
                    end
                end
                
                % create new
                merge_obs = obs_merge_simple(upObj_sf); % draw counturing object
            ...
            elseif d==3
                set(sp.obs(n),'XData',get(sp.obs(n),'XData')+ dx(1))
                set(sp.obs(n),'YData',get(sp.obs(n),'YData')+ dx(2))
                set(sp.obs(n),'ZData',get(sp.obs(n),'ZData')+ dx(3))
            end
        end
