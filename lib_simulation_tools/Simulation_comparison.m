...
if options.saveAnimation
    ...
    vidObj1 = VideoWriter(strcat('animations/', figName)); % TODO
...
if options.plot %plotting options
    if isfield(options,'figure')
        ...
        title('Orthogonal Modulation Matrix')
        
        sp{2}.fig = subplot(1,2,2);
        title('Center Based Modulation')
    else
        sp = [];
    end
...
while true
    ...
    if options.saveAnimation
        ...
        frameTemp = getframe(options.figure);
        writeVideo(vidObj1, frameTemp);
        %woptions
        %writeVideo(vidObj, gcf);
    end
