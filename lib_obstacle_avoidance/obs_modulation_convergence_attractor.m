function [xd, M, compTime] = obs_modulation_convergence_attractor(x,xd,obs, ds_handle, varargin)
%
...
for i=1:length(varargin)
    if ~isempty(varargin)
        switch i % Assign input arguments
            ...
            case 3 % Fixed points where initial dynamics should be kept
                fixed_points = varargin{3}; 
                if size(fixed_points,1)
                    % Gamma of fixed points is 
                    Gamma_fixedPoints = sqrt(sum((x-fixed_points).^2,1));
                    xd_modulated = [xd_modulated,repmat(xd,1,size(fixed_points,2))]; 

                end
            case 4 % DS type -- slightly different algorithm for linear/nonlinear DS
                ds_type = varargin{4}; 
        end
    end
end
...
if dim==3
    ...
    E_normal = zeros(dim,dim+1,N_obs);
else
    E = zeros(dim,dim,N_obs);
    E_normal = zeros(dim,dim,N_obs);
end
...
%     obs{n}.dx_center = ds_handle(obs{n}.x0);
    ...
    [E(:,:,n), Gamma(n), E_normal(:,:,n)] = compute_basis_matrix(dim,x_alligned,obs{n},R(:,:,n));
%     if Gamma(n)<0.99
%         disp(Gamma(n))
%     end
    
    if strcmp(ds_type,'nonlinear')
        % Caclulate DS hat
        x_hat = compute_replacement_position(dim,x_alligned,obs{n},R(:,:,n), Gamma(n)); % TODO -- maybe include in previous function
        dx_init(:,n) = ds_handle(x_hat);
    else
        dx_init(:,n) = xd;
    end
end
...
%     D = w(n)*([-1;d0]/abs(Gamma(n))^(1/rho)); -- REMOVED weight on D
    D = ([-1;d0]/abs(Gamma(n))^(1/rho));
...
%     M = (R(:,:,n)*E(:,:,n)*diag(D+[1;d0])/E(:,:,n)*R(:,:,n)');
    ...
    % Apply on desired (replacement) DS
    xd = dx_init(:,n);
    ...
    xd_modulated(:,n) = xd + xd_obs; %transforming back the velocity into the global coordinate system
end

% Includes attractor
xd = sum(xd_modulated.*repmat(w,size(xd_modulated,1),1),2);

% No attractor - TODO remove
% xd = sum(xd_modulated(:,1:N_obs).*repmat(w,N_obs,1),2);
xd = xd_modulated(:,1); %only for one obstacel
...
function [E, Gamma, E_normal] = compute_basis_matrix(d,x_t,obs, R)
