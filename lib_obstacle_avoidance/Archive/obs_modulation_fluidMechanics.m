...
% Extract varying arguments
if ~isempty(varargin)
    for i=1:max(length(varargin),2)
        switch i
            case 1
                xd_obs = varargin{1};
            case 2
...
% I
if isfield(varargin,'fig_handle')
    fig_handle = varargin.fig_hanlde;
    showPlot = true;
else
    showPlot = false;
end
...
for ind = indDistObj
    ...
    if(isfield(obs{ind},'concaveAngle'))
        xd = spaceTrafo_concaveEllipse(obs{ind}, x, xd, w_obs(ind));
    else
...
function [xd] = spaceTrafo_concaveEllipse(obs, x, xd, w_obs)
% Recenter
x = x - obs.x0;
